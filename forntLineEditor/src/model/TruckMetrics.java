...
public class TruckMetrics {
	...
	public ArrayList<String> getTrucks() {
			...
			try {
			    ...
			    URL url = new URL("http://www.military-today.com/trucks.htm");
			    ...
			    Boolean truckAct = false;

	            while ((line = reader.readLine()) != null) {
	            		            	
	            	if(line.contains("Light Utility Vehicles")) { 
	            		
	            		truckAct = true;
	            		
	            	}
	            	
	            	if(truckAct == true && line.contains("alt=") && !line.contains("flags")) {
	            		
	            		String[] parts = line.split("alt=\"");
	            		parts[1] = parts[1].substring(0,parts[1].indexOf("\""));
	            			            		
	            		if(parts[1].equals("Fast Attack and Special Forces Vehicles") || parts[1].contains("Intellectus Publishing ")) {
	            			
	            			return arTrucks;
	            			
	            		}
		                            		
	            		arTrucks.add(parts[1]);
